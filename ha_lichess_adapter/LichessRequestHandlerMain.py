...
LICHESS_SINGLE_CALL_PARAMETER_IN_SENSOR = 'sensor.chessboard_lichess_api_call_main'
...
class LichessRequestHandlerMain(hass.Hass):
    ...
    def initialize(self):        
        self.log("AppDaemon LichessRequestHandler (main) script initialized!")
        ...
        # we are ready to go
        self.log(f"Waiting for new api call (main)")
    ...
    def game_id_changed(self, entity, attribute, old, new, kwargs):
        if new and new != self.__class__._current_game_id:
            self.log(f"Game ID changed (main request): {self.__class__._current_game_id} -> {new}")
    ...
    def token_changed_main(self, entity, attribute, old, new, kwargs):
        if new  and new != UNAVAILABLE_STATE and new != UNKNOWN_STATE and new != EMPTY_STRING and self.decrypt_message(new) != self.__class__._current_token_main:
            ...
            self.log(f"Main token changed (request main): {self.__class__._current_token_main} -> {new_decrypted}")
    ...
    def token_changed_opponent(self, entity, attribute, old, new, kwargs):
        if new  and new != UNAVAILABLE_STATE and new != UNKNOWN_STATE and new != EMPTY_STRING and self.decrypt_message(new) != self.__class__._current_token_opponent:
            new_decrypted = self.decrypt_message(new)
            self.log(f"Opponent token changed (main request for opponent): {self.__class__._current_token_opponent} -> {new_decrypted}")
